...
/// `GET /tasks` filters.
///
/// `deny_unknown_fields` is load-bearing: without it, `?queeue=agent` is
/// silently ignored and the caller gets the human list with a 200. For queries
/// generated by an agent skill, answering a different question than the one
/// asked is worse than refusing. Safe to be strict here because the protocol
/// is internal and both binaries ship together (SPEC.md §6.6).
